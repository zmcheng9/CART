...
from models.fewshot_cart import FewShotSeg
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                for sub_chunck in range(_config['n_part']):  # n_part = 3
                    ...
                    for i in range(query_image_s.shape[0]):
                        ...
                        k = len(_pred_s)
                        _pred_s = torch.sum(_pred_s, dim=0)  # 1 x H x W
                        _pred_s = torch.where(_pred_s >= (k / 2), 1, 0).unsqueeze(0)    # 1 x h x w
                        query_pred_s.append(_pred_s)
                    query_pred_s = torch.cat(query_pred_s, dim=0)  # [9, 2, 256, 256]  [9, 256, 256]
